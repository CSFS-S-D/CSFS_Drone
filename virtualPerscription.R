...
# Project Boundaries
bnds = st_read("../../Pinecliffe_BUFO/Arc/Products/Vector/bounds.gpkg")
pondo_bnds = bnds[bnds$Stand=="Open Ponderosa",]

# tree map
ttops = st_read("../../Pinecliffe_BUFO/Arc/Products/Vector/ttops_c.gpkg")
ttops = st_crop(ttops, pondo_bnds)

# load an las file
las_list = dir("../../Pinecliffe_BUFO/Arc/Products/las_segmented/", full.names = T)
las = readLAS(las_list)
...
tree_clump_dist_m <- 6
ostory_dbh_cm <- 6*2.54 # = inches*2.54. 4 because lots of small dbh juniper
...
# desired BA
target_ba = 15 # cannot be > current BA
# desired QMD
target_qmd = 12
